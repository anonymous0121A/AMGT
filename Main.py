...
class Coach:
	...
	def trainEpoch(self):
		...
		for i, tem in enumerate(trnLoader):
			...
			lastLosses.append(bprLoss + contrastLoss)
			
			if i % args.fixSteps == 0:
				localGlobalLoss = -sampScores.mean() * calcReward(lastLosses, args.eps)
			...
			loss.backward()
			self.opt.step()
			log('Step %d/%d: loss = %.3f, regLoss = %.3f, clLoss = %.3f        ' % (i, steps, loss, regLoss, contrastLoss), save=False, oneline=True)
		ret = dict()
